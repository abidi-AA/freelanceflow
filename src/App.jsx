    ...
    <Modal onClose={onClose}>
      <div className="upgrade-modal">
        ...
        <div className="upgrade-list">
          {["Unlimited clients & invoices","CSV export for your accountant","Cloud backup — never lose data","Invoice PDF download","Priority support"].map(f => (
            <div key={f} className="upgrade-item">
              <span className="upgrade-check">✓</span><span>{f}</span>
            </div>
          ))}
        </div>
...
export default function App() {
  ...
  function addInvoice() {
    ...
    if (!client) return;
    persist({ ...d, invoices: [{ id:uid(), number:nextInvNum(d.invoices), clientId:form.clientId, clientName:client.name, desc:form.desc, amount:Number(form.amount), due:form.due, status:"draft", date:new Date().toISOString().slice(0,10) }, ...d.invoices] });
  ...
  function updateStatus(id, status) { persist({ ...d, invoices: d.invoices.map(i => i.id === id ? { ...i, status } : i) }); }
  ...
  const PAGES = { dashboard:"Dashboard", invoices:"Invoices", clients:"Clients", tax:"Tax Estimate" };
  const NAV   = [{ key:"dashboard", icon:"📊", label:"Dashboard" },{ key:"invoices", icon:"🧾", label:"Invoices" },{ key:"clients", icon:"👤", label:"Clients" },{ key:"tax", icon:"📋", label:"Tax" }];

  const monthlyData = (() => {
    const map = {};
    paid.forEach(i => { const m=(i.date||"").slice(0,7); if(m) map[m]=(map[m]||0)+i.amount; });
    return Object.entries(map).sort((a,b)=>a[0].localeCompare(b[0])).map(([m,v])=>({ label:new Date(m+"-01").toLocaleDateString("en",{month:"short",year:"numeric"}), amount:v }));
  })();
    ...
    <div className="shell">
      ...
      <aside className="sidebar">
        <div className="sidebar-brand">
          <img src="/logo.svg" alt="FreelanceFlow" style={{width:"100%",borderRadius:"10px",display:"block"}} />
        </div>
      ...
      <div className="main">
        <header className="topbar">
          ...
          <div className="topbar-actions">
            ...
            {tab==="invoices" && isProEligible && <button className="btn btn-ghost btn-sm" onClick={exportCSV}>↓ CSV</button>}
            {tab!=="tax" && (
        ...
        <div className="page">
          ...
          {tab==="dashboard" && !isNewUser && (
            <>
              <div className="metrics-row">
                {[
                  {label:"Total earned", val:fmt(totalEarned), sub:`${paid.length} paid invoices`, cls:"c-green"},
                  {label:"Outstanding",  val:fmt(outstanding),  sub:`${d.invoices.filter(i=>i.status==="sent"||i.status==="overdue").length} awaiting`, cls:"c-amber"},
                  {label:"Overdue",      val:fmt(overdueAmt),   sub:`${d.invoices.filter(i=>i.status==="overdue").length} past due`, cls:"c-red"},
                  {label:"Tax aside",    val:fmt(taxOwed),      sub:`${d.taxRate}% of income`, cls:"c-muted"},
                ].map(m=>(
              ...
              <div className="net-card">
                <div><div className="label">Net take-home after tax</div><div className="val">{fmt(netIncome)}</div></div>
                <div style={{textAlign:"right"}}><div className="label">This month</div><div className="val-sm">{fmt(monthEarned)}</div></div>
              </div>
              ...
              <div className="table-wrap">
                {d.invoices.length===0
                  ? <div className="empty"><div className="empty-icon">🧾</div><div className="empty-title">No invoices yet</div><button className="btn btn-primary" onClick={()=>{ setModal("invoice"); setForm({}); }}>+ New invoice</button></div>
                  : <table>
            ...
            <>
              ...
              <div className="table-wrap">
                {filteredInvoices.length===0
                  ? <div className="empty"><div className="empty-icon">🧾</div><div className="empty-title">No {filter!=="all"?filter+" ":""}invoices</div>{filter==="all"&&<button className="btn btn-primary" onClick={()=>{ setModal("invoice"); setForm({}); }}>+ New invoice</button>}</div>
                  : <table>
                      <thead><tr><th>#</th><th>Client</th><th>Description</th><th>Amount</th><th>Due</th><th>Status</th><th></th></tr></thead>
                      <tbody>
                        {filteredInvoices.map(inv=>(
                          <tr key={inv.id}>
                            <td><span className="inv-num">{inv.number||"—"}</span></td>
                            <td><div className="td-main">{inv.clientName}</div><div className="td-sub">{inv.date}</div></td>
                            <td>{inv.desc}</td>
                            <td style={{fontWeight:600}}>{fmt(inv.amount)}</td>
                            <td style={{color:"var(--text-3)"}}>{inv.due}</td>
                            <td>
              ...
              {filteredInvoices.length>0 && <div style={{textAlign:"right",marginTop:"10px",fontSize:"13px",color:"var(--text-3)"}}>Total: <strong>{fmt(filteredInvoices.reduce((s,i)=>s+i.amount,0))}</strong></div>}
            </>
          )}
          ...
          {tab==="clients" && (
            <>
              {d.clients.length===0
                ? <div className="empty" style={{paddingTop:"80px"}}><div className="empty-icon">👤</div><div className="empty-title">No clients yet</div><div className="empty-sub">Add your first client to start creating invoices.</div><button className="btn btn-primary" onClick={()=>{ setModal("client"); setForm({}); }}>+ Add first client</button></div>
                      ...
                      return (
                        <div key={c.id} className="client-card">
                          <div className="client-head">
                            ...
                            <div><div className="client-name">{c.name}</div><div className="client-email">{c.email||"No email"}</div></div>
                          </div>
                          ...
                          <button className="btn btn-danger" style={{width:"100%",marginTop:"4px"}} onClick={()=>deleteClient(c.id)}>Remove</button>
                        </div>
                      );
                    })}
                    <div className="card client-add-card" onClick={()=>{ setModal("client"); setForm({}); }}>
                      <div className="client-add-icon">+</div><span style={{fontSize:"13px"}}>Add client</span>
                    </div>
                  </div>
              }
            </>
            ...
            <>
              <div className="card card-p" style={{marginBottom:"20px"}}>
                ...
                <div className="slider-wrap">
                  <input type="range" min="10" max="50" step="1" className="slider" value={d.taxRate} onChange={e=>persist({...d,taxRate:Number(e.target.value)})}/>
        ...
        <Modal onClose={()=>{ setModal(null); setForm({}); }}>
          ...
          {d.clients.length===0
            ...
            : <>
                ...
                {[{label:"Description",field:"desc",type:"text",placeholder:"What did you deliver?"},{label:"Amount (USD)",field:"amount",type:"number",placeholder:"0"},{label:"Due date",field:"due",type:"date"}].map(f=>(
                  <div key={f.field} className="field">
                    <label>{f.label}</label>
                    <input className="input" type={f.type} placeholder={f.placeholder||""} value={form[f.field]||""} onChange={e=>setForm({...form,[f.field]:e.target.value})}/>
                  </div>
                ))}
                {form.amount&&Number(form.amount)>0&&(
                  <div className="tax-hint">Tax ({d.taxRate}%): <strong style={{color:"var(--red)"}}>{fmt(Math.round(Number(form.amount)*d.taxRate/100))}</strong> · You keep: <strong>{fmt(Math.round(Number(form.amount)*(1-d.taxRate/100)))}</strong></div>
                )}
      ...
      {modal==="client" && (
        <Modal onClose={()=>{ setModal(null); setForm({}); }}>
          ...
          {[{label:"Name *",field:"name",placeholder:"Client or company name"},{label:"Email",field:"email",placeholder:"billing@client.com"},{label:"Company",field:"company",placeholder:"Optional"}].map(f=>(
            <div key={f.field} className="field">
              <label>{f.label}</label>
              <input className="input" placeholder={f.placeholder} value={form[f.field]||""} onChange={e=>setForm({...form,[f.field]:e.target.value})}/>
            </div>
          ))}
          <div className="modal-footer">
